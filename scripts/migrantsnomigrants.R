...
(plot2 <- ggplot(indivPre, aes(EASTING,NORTHING)) + 
    geom_point(data = indivPre, color = 'green', size = 1, alpha = 0.8) +
    geom_point(data = indivMig, color = 'blue', size = 1, alpha = 0.8) +
    geom_point(data = indivPost, color = 'grey', size = 1, alpha = 0.8)
)

###Plot spring migration with mvt and encamp behavior
ggplot(out[Animal_ID == "mr2011a01"],aes(Easting, Northing)) +
