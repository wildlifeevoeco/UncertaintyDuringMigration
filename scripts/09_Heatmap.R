...
m1 <-
  mclogit(cbind(Randoms,PtID)~(Forest)*((prcp) + (swe) + (tmax)) + (Lichen)*((prcp)+(swe)+(tmax))+(Forest) + (Wetland) + (Lichen) + (Rocky) + (Water) + (tmax) + (prcp) + (swe) + (NDVI), data = allNDVI_stop)

m2 <-
  mclogit(cbind(Randoms,PtID)~(Forest)*((prcp) + (swe) + (tmax)) + (Lichen)*((prcp)+(swe)+(tmax))+(Forest) + (Wetland) + (Lichen) + (Rocky) + (Water) + (tmax) + (prcp) + (swe) + (NDVI), data = allNDVI_mvt)
...
### This script is for two interactions (FPT and KDE), so you only need to run one if only making a single plot
HeatmapdataFPT <- data.frame(
  x = rep(seq(min(allNDVI_stop$Lichen, na.rm = FALSE),
              max(allNDVI_stop$Lichen, na.rm = FALSE),
              by = ((max(allNDVI_stop$Lichen, na.rm = FALSE) - min(allNDVI_stop$Lichen, na.rm = FALSE))/200)), 201),
  y = rep(seq(min(allNDVI_stop$tmax, na.rm = FALSE),
              max(allNDVI_stop$tmax, na.rm = FALSE),
              by = ((max(allNDVI_stop$tmax, na.rm = FALSE) - min(allNDVI_stop$tmax, na.rm = FALSE))/200)), each = 201))

HeatmapdataKDE <- data.frame(
  x = rep(seq(min(allNDVI_mvt$Lichen, na.rm = FALSE),
              max(allNDVI_mvt$Lichen, na.rm = FALSE),
              by = ((max(allNDVI_mvt$Lichen, na.rm = FALSE) - min(allNDVI_mvt$Lichen, na.rm = FALSE))/200)), 201),
  y = rep(seq(min(allNDVI_mvt$tmax, na.rm = FALSE),
              max(allNDVI_mvt$tmax, na.rm = FALSE),
              by = ((max(allNDVI_mvt$tmax, na.rm = FALSE) - min(allNDVI_mvt$tmax, na.rm = FALSE))/200)), each = 201))
...
## I think you can drop this if it's scaled it should be zero anyway.
## 2 = FPT scaled, 5 and 6 are the interactions
HeatmapdataFPT$z <- (((coef(m1)[5] * HeatmapdataFPT$x) +
                       (coef(m1)[4] * HeatmapdataFPT$y) +
                       (coef(m1)[15] * (HeatmapdataFPT$x) * (HeatmapdataFPT$y))))

HeatmapdataKDE$z <- (((coef(m1)[5] * HeatmapdataKDE$x) +
                        (coef(m1)[4] * HeatmapdataKDE$y) +
                        (coef(m1)[15] * (HeatmapdataKDE$x) * (HeatmapdataKDE$y))))
...
labelx<-seq(0,1,0.2)
labely<-seq(-13,18,2)
LichenLoc<-(labelx-(min(allNDVI_stop$Lichen)))/(max(allNDVI_stop$Lichen)-min(allNDVI_stop$Lichen))
FPTLoc<-(labely-(min(allNDVI_stop$tmax)))/(max(allNDVI_stop$tmax)-min(allNDVI_stop$tmax))

labelx<-seq(0,1,0.2)
labely<-seq(-13,18,2)
LichenLoc2<-(labelx-(min(allNDVI_mvt$Lichen)))/(max(allNDVI_mvt$Lichen)-min(allNDVI_mvt$Lichen))
KDELoc<-(labely-(min(allNDVI_mvt$tmax)))/(max(allNDVI_mvt$tmax)-min(allNDVI_mvt$tmax))
...
### Output ----
png("graphics/Fig3_Heatmap_Lichentemp1.png", width = 6.81, height = 3.4, units = "in",res=300)
par(mar = c(3.5, 3, 1, 0), mfrow = c(1, 2))
legend_im <- as.raster(matrix(rev(rbPal), ncol = 1))
image(HeatmapFPT, col = rbPal, axes = FALSE, xlim = c(0, 1.15),xlab=NA,
      zlim = c(min(c(minValue(HeatmapFPT))), maxValue(HeatmapFPT)))
axis(1,at=LichenLoc,labels=labelx,cex.axis=0.8)
segments(0,0,1,0)
axis(2,at=FPTLoc,labels=labely,cex.axis=0.8, las = 1)
axis(2,at=c(-100,100),labels=NA,cex.axis=0.8)
mtext("Proportion of Lichen stopover", side = 1, padj = 3, adj = 0.4)
mtext("Variation of temperature", side = 2, padj = 2, outer = T)
mtext("A", side = 3,  at = 0.05)
par(mar = c(3.5, 1.5, 1, 0))

png("graphics/Fig4_Heatmap_Lichentemp_mvt.png", width = 6.81, height = 3.4, units = "in",res=300)
image(HeatmapKDE, col = rbPal, axes = FALSE, xlim = c(0, 1.25), xlab=NA,
      zlim = c(min(c(minValue(HeatmapKDE), minValue(HeatmapFPT))),
               max(c(maxValue(HeatmapKDE), maxValue(HeatmapFPT)))))
axis(1,at=LichenLoc2,labels=labelx,cex.axis=0.8)
axis(2,at=KDELoc,labels=labely,cex.axis=0.8, las = 1)
axis(2,at=c(-100,100),labels=NA,cex.axis=0.8)
segments(0,0,1,0)
mtext("Proportion of Lichen movement", side = 1, padj = 3, adj = 0.4)
rasterImage(legend_im, 1.02, 0.2, 1.09, 0.8)
mtext("B", side = 3,  at = 0.05)
par(mar = c(3.5, 1.5, 1, 0))
#text(x = 1.18, y = 0.8, round(max(c(maxValue(HeatmapKDE), maxValue(HeatmapFPT))), digits = 2))
